...
void Emulator::ExecuteOpcode(BYTE opcode)
{
	switch(opcode)
	{
		...
		//Add signed byte value into register sp, Flags(0,0,H,C)
		case 0xE8:
		{
			char n = (signed char)ReadMemory(m_ProgramCounter);
			m_ProgramCounter++;
			if ((m_StackPointer.reg & 0xF) + (n & 0xF) >= 0x10)
				m_RegisterAF.lo = BitSet(m_RegisterAF.lo, FLAG_H);
			else
				m_RegisterAF.lo = BitReset(m_RegisterAF.lo, FLAG_H);
			if ((m_StackPointer.reg + n) > 0xFFFF)
